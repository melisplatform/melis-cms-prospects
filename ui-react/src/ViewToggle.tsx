 ...
 * de MelisCore. Styles inline + variables CSS du thème. La page hôte gère l'état `mode` et
 * le montage de l'iframe « Old ». */
...
// `compact` (défaut false) : masque les libellés « New »/« Old » (icônes + tooltip seulement),
// pour le header en vue mobile — chaque site d'appel existant garde son comportement (défaut false).
export function ViewToggle({ mode, onChange, compact = false }: { mode: ViewMode; onChange: (m: ViewMode) => void; compact?: boolean }) {
  ...
  return (
    <div style={{ display: 'inline-flex', gap: 4, padding: 4, borderRadius: 8, border: '1px solid var(--color-border)', background: 'color-mix(in srgb, var(--color-muted,#888) 12%, transparent)' }}>
      <button style={tab(mode === 'react')} onClick={() => onChange('react')} title={compact ? 'New (React)' : undefined}><SparkIcon />{!compact && 'New'}</button>
